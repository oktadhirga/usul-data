...
<div class="max-w-4xl mx-auto space-y-6">
	...
	<div class="space-y-4">
		...
		{#if isEditMode}
			<div class="rounded-xl border border-amber-500/30 bg-amber-500/10 p-3.5 flex items-center justify-between text-xs text-amber-300">
				<div class="flex items-center gap-2">
					<Pencil class="h-4 w-4 shrink-0 text-amber-400" />
					<span>
				...
				<div class="flex flex-col gap-1.5">
					<div
						class={`h-1.5 rounded-full transition-all duration-300 ${
							currentStep >= step.num ? 'bg-indigo-600' : 'bg-slate-800'
						}`}
					></div>
					<span
	...
	{#if alertMessage}
		<Alert variant={alertMessage.type} class="border shadow-lg">
			<div class="flex items-start gap-3">
				...
				{:else}
					<AlertCircle class="h-5 w-5 text-rose-400 mt-0.5 shrink-0" />
				{/if}
				<div class="flex-1">
					<AlertTitle class="font-semibold text-white">{alertMessage.title}</AlertTitle>
	...
	<Card class="border-slate-800 bg-slate-900/60 backdrop-blur-md">
		<!-- STEP 1: PILIH PEGAWAI -->
		{#if currentStep === 1}
			<CardHeader class="border-b border-slate-800/80 p-6 sm:p-7 pb-4">
			...
			<CardContent class="space-y-6 p-6 sm:p-7 pt-6">
				...
				<div class="space-y-2">
					<Label class="text-sm font-semibold text-slate-300">Pencarian Pegawai:</Label>
					<div class="flex items-center w-full h-12 rounded-xl border border-slate-800 bg-slate-950 px-4 gap-3.5 focus-within:ring-2 focus-within:ring-indigo-500 focus-within:border-indigo-500 transition-all">
						<Search class="h-5 w-5 text-slate-400 shrink-0" />
						<input
							type="text"
							placeholder="Cari berdasarkan nama, NIP, atau jabatan..."
							bind:value={searchPegawai}
							class="w-full h-full bg-transparent text-slate-200 text-sm focus:outline-none placeholder:text-slate-500 border-none p-0"
						/>
					</div>
				</div>
				...
				<div class="space-y-2.5">
					<div class="flex items-center justify-between">
						<Label class="text-sm font-semibold text-slate-300">Daftar Pegawai:</Label>
					...
					{:else if filteredPegawai().length === 0}
						<div class="py-16 text-center rounded-xl border border-slate-800 bg-slate-950/40">
							<p class="text-sm text-slate-400">Pegawai tidak ditemukan.</p>
								...
								<button
									...
									class={`w-full text-left p-4 sm:p-5 rounded-xl border transition-all flex items-center justify-between gap-4 cursor-pointer ${
										selectedPegawai?.id === p.id
											? 'border-indigo-500 bg-indigo-600/10 ring-2 ring-indigo-500/50 shadow-md shadow-indigo-500/10'
								...
								>
									<div class="space-y-2 flex-1 min-w-0">
										<div class="flex items-center gap-2.5 flex-wrap">
											<p class="font-bold text-base text-slate-100 truncate">{p.nama}</p>
		...
		{#if currentStep === 2}
			...
			<CardContent class="space-y-7 p-6 sm:p-7 pt-6">
				...
				<div class="space-y-3">
					<Label class="text-sm font-semibold text-slate-300">Jenis Usulan:</Label>
							...
							<button
								...
								class={`p-5 rounded-xl border text-left transition-all cursor-pointer ${
									selectedJenis === opt.value
										? 'border-indigo-500 bg-indigo-600/10 ring-2 ring-indigo-500/40 shadow-sm'
										: 'border-slate-800 bg-slate-950/40 hover:border-slate-700 hover:bg-slate-800/30'
								}`}
								onclick={() => (selectedJenis = opt.value)}
							>
								<div class="flex items-center justify-between mb-1.5">
									<span class="font-bold text-sm text-slate-200">{opt.label}</span>
								...
								<div class="mt-3.5">
									{#if opt.value === 'hapus'}
										<Badge variant="outline" class="text-[10px] border-slate-700 text-slate-400 px-2 py-0.5">
											Dokumen Opsional
										</Badge>
									{:else}
										<Badge variant="outline" class="text-[10px] border-amber-500/30 text-amber-400 bg-amber-500/10 px-2 py-0.5">
											Wajib Upload PDF
										</Badge>
									{/if}
								</div>
				...
				<div class="space-y-2.5">
					<Label for="kategori-select" class="text-sm font-semibold text-slate-300">Kategori Data:</Label>
					<p class="text-xs text-slate-400">Pilih rumpun kategori data yang akan diajukan perubahannya:</p>
					<div class="relative mt-1">
						<select
							id="kategori-select"
							bind:value={selectedKategori}
							class="w-full h-12 px-4 py-3 rounded-xl border border-slate-800 bg-slate-950 text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 cursor-pointer appearance-none pr-10"
						>
							{#each kategoriOptions as kat}
								<option value={kat} class="bg-slate-900 text-slate-200 py-2.5 px-4">{kat}</option>
							{/each}
						</select>
			...
			<CardContent class="space-y-6 p-6 sm:p-7 pt-6">
				<div class="space-y-3">
					{#each fields as field, index (field.id)}
						<div class="rounded-xl border border-slate-800 bg-slate-950/50 p-4 space-y-3 relative group">
							<div class="flex items-center justify-between">
								<span class="text-xs font-bold text-indigo-400">Field #{index + 1}</span>
								{#if fields.length > 1}
									<Button
										variant="ghost"
										size="sm"
										class="h-7 text-xs text-rose-400 hover:text-rose-300 hover:bg-rose-500/10"
							...
							<div class="grid grid-cols-1 md:grid-cols-3 gap-3">
								<div class="space-y-1">
									<Label class="text-xs text-slate-400">Nama Field / Elemen Data *</Label>
									<Input
										type="text"
										placeholder="contoh: Nama Lengkap / Gelar"
										bind:value={field.fieldName}
										class="border-slate-800 bg-slate-900 text-slate-200 text-xs"
									/>
								</div>

								<div class="space-y-1">
									<Label class="text-xs text-slate-400">Nilai Lama (Sebelumnya)</Label>
									<Input
										type="text"
								...
								<div class="space-y-1">
									<Label class="text-xs text-slate-400">Nilai Baru (Usulan)</Label>
									<Input
										type="text"
										placeholder="contoh: S2 Manajemen Sistem Informasi"
										bind:value={field.nilaiBaru}
										class="border-slate-800 bg-slate-900 text-slate-200 text-xs font-semibold text-emerald-400"
									/>
								</div>
							</div>
						</div>
					...
					<Button
						...
						variant="outline"
						size="sm"
						class="w-full border-dashed border-slate-700 bg-slate-900/40 hover:bg-slate-800 text-slate-300 text-xs py-2"
						onclick={addFieldRow}
					>
				...
				<div class="space-y-1 pt-2">
					<Label class="text-xs text-slate-300">Catatan Tambahan Pengusul (Opsional):</Label>
					<textarea
						bind:value={catatanPengusul}
						rows="3"
						placeholder="Berikan keterangan atau alasan perubahan data bila diperlukan..."
						class="w-full rounded-md border border-slate-800 bg-slate-950 px-3 py-2 text-xs text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500"
					></textarea>
				</div>
			</CardContent>
		{/if}
		...
		{#if currentStep === 4}
			<CardHeader class="border-b border-slate-800/80 p-6 sm:p-7 pb-4">
				<CardTitle class="text-lg text-white flex items-center gap-2">
					<Upload class="h-5 w-5 text-indigo-400" />
			...
			<CardContent class="space-y-6 p-6 sm:p-7 pt-6">
				...
				{#if isDocumentRequired}
					<div class="flex items-start gap-2.5 rounded-lg border border-amber-500/20 bg-amber-500/10 p-3 text-xs text-amber-300">
				...
				{:else}
					<div class="flex items-start gap-2.5 rounded-lg border border-slate-700/50 bg-slate-800/30 p-3 text-xs text-slate-300">
						<Info class="h-4 w-4 mt-0.5 shrink-0" />
						<div>
				...
				{#if existingDocuments && existingDocuments.length > 0}
					<div class="rounded-xl border border-slate-800 bg-slate-950/40 p-3.5 space-y-2">
						<span class="text-xs font-semibold text-slate-300">Dokumen Pendukung Yang Sudah Diunggah:</span>
						{#each existingDocuments as doc}
							<div class="flex items-center justify-between rounded-lg border border-slate-800 bg-slate-900/60 p-2.5 text-xs">
								<div class="flex items-center gap-2 overflow-hidden">
									<FileText class="h-4 w-4 text-emerald-400 shrink-0" />
									<span class="text-slate-200 truncate font-medium">{doc.namaDokumen}</span>
								...
								<a
									...
									class="inline-flex items-center gap-1 rounded bg-slate-800 hover:bg-slate-700 px-2 py-0.5 text-xs text-indigo-300 transition-colors shrink-0 ml-2"
								>
									Lihat Berkas
								</a>
							</div>
				...
				<div class="space-y-2">
					...
					<div class="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-700 bg-slate-950/40 p-6 hover:border-indigo-500 transition-colors">
						<Upload class="h-8 w-8 text-indigo-400 mb-2" />
						<p class="text-xs font-semibold text-slate-300">Pilih berkas dari perangkat Anda</p>
						...
						<input
							...
							class="mt-4 text-xs text-slate-400 file:mr-3 file:py-1.5 file:px-3 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-indigo-600 file:text-white hover:file:bg-indigo-500 cursor-pointer"
						/>
					</div>

					{#if fileError}
						<p class="text-xs text-rose-400 font-medium">{fileError}</p>
					{/if}
				</div>
					...
					<div class="rounded-xl border border-indigo-500/30 bg-indigo-500/5 p-4 space-y-3">
						<div class="flex items-center justify-between">
							<div class="flex items-center gap-2">
								<FileText class="h-5 w-5 text-indigo-400" />
								<div>
									<p class="text-xs font-semibold text-white">{attachedFile.name}</p>
									<p class="text-[10px] text-slate-400">
										{(attachedFile.size / 1024).toFixed(1)} KB • application/pdf
									</p>
								</div>
							</div>
							<Badge class="bg-emerald-500/10 text-emerald-400 border-emerald-500/30 text-[10px]">
								Valid PDF
							</Badge>
						</div>

						<div class="space-y-1">
							<Label class="text-xs text-slate-400">Judul / Keterangan Dokumen:</Label>
							<Input
								type="text"
			...
			<CardContent class="space-y-3.5 p-5 sm:p-6 pt-4">
				...
				{#if selectedPegawai}
					<div class="rounded-xl border border-slate-800 bg-slate-950/60 p-4 space-y-2">
						<span class="text-xs font-bold text-indigo-400 uppercase tracking-wider">Pegawai Terkait</span>
						<div class="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
							<div class="space-y-0.5">
								<span class="text-slate-500">Nama Lengkap:</span>
								<p class="font-bold text-slate-100">{selectedPegawai.nama}</p>
							</div>
							<div class="space-y-0.5">
								<span class="text-slate-500">NIP:</span>
								<p class="font-mono text-indigo-300">{selectedPegawai.nip}</p>
							</div>
							<div class="space-y-0.5">
								<span class="text-slate-500">Jabatan:</span>
								<p class="text-slate-300">{selectedPegawai.jabatan}</p>
							</div>
							<div class="space-y-0.5">
								<span class="text-slate-500">Unit Organisasi:</span>
								<p class="text-slate-300">{selectedPegawai.namaUnor || selectedPegawai.kodeUnor}</p>
							</div>
						</div>
					</div>
				{/if}
				...
				<div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
					<div class="rounded-xl border border-slate-800 bg-slate-950/50 p-3.5 space-y-0.5">
						<span class="text-xs font-semibold text-slate-400">Jenis Tindakan:</span>
						<p class="text-xs font-bold text-slate-100 uppercase">{selectedJenis}</p>
					</div>
					<div class="rounded-xl border border-slate-800 bg-slate-950/50 p-3.5 space-y-0.5">
						<span class="text-xs font-semibold text-slate-400">Kategori Data:</span>
						<p class="text-xs font-bold text-indigo-300">{selectedKategori}</p>
					</div>
				</div>
				...
				<div class="rounded-xl border border-slate-800 bg-slate-950/50 p-4 space-y-2.5">
					<span class="text-xs font-bold text-slate-300 uppercase tracking-wider">Rincian Perubahan Field:</span>
					<div class="rounded-lg border border-slate-800 overflow-hidden text-xs bg-slate-900/60">
							...
							<tbody class="divide-y divide-slate-800/60 bg-slate-950/20">
								{#each fields.filter((f) => f.fieldName.trim().length > 0) as f}
									<tr class="hover:bg-slate-900/30 transition-colors">
										<td class="py-2.5 px-3.5 font-mono text-slate-200 font-medium">{f.fieldName}</td>
										<td class="py-2.5 px-3.5 text-rose-300 line-through font-mono">{f.nilaiLama || '-'}</td>
										<td class="py-2.5 px-3.5 text-emerald-400 font-bold font-mono">{f.nilaiBaru || '-'}</td>
									</tr>
								{/each}
							</tbody>
						</table>
				...
				<!-- Ringkasan Dokumen -->
				<div class="rounded-xl border border-slate-800 bg-slate-950/50 p-3.5 space-y-2">
					<span class="text-xs font-bold text-slate-300 uppercase tracking-wider">Dokumen Pendukung:</span>
					{#if attachedFile}
						<div class="flex items-center gap-2.5 rounded-lg border border-slate-800 bg-slate-900/70 p-2.5 text-xs text-indigo-300">
							<FileText class="h-4 w-4 shrink-0 text-indigo-400" />
							<div>
								<p class="font-semibold text-slate-100">{documentName || attachedFile.name} (Dokumen Baru)</p>
					...
					{#if existingDocuments && existingDocuments.length > 0}
						<div class="space-y-1.5">
							{#each existingDocuments as doc}
								<div class="flex items-center gap-2.5 rounded-lg border border-slate-800 bg-slate-900/70 p-2 text-xs text-indigo-300">
									<FileText class="h-4 w-4 shrink-0 text-emerald-400" />
									<div>
										<p class="font-semibold text-slate-200">{doc.namaDokumen}</p>
				...
				{#if catatanPengusul}
					<div class="rounded-xl border border-slate-800 bg-slate-950/50 p-3.5 space-y-1.5">
						<span class="text-xs font-bold text-slate-300 uppercase tracking-wider">Catatan Pengusul:</span>
						<p class="text-xs text-slate-300 whitespace-pre-wrap leading-relaxed">{catatanPengusul}</p>
					</div>
				{/if}
			</CardContent>
		{/if}
		...
		<CardFooter class="flex items-center justify-between border-t border-slate-800/80 p-6 sm:p-7 pt-5">
			<div>
				{#if currentStep > 1}
					<Button
						variant="outline"
						size="sm"
						class="border-slate-700 bg-slate-800 hover:bg-slate-700 text-slate-200"
