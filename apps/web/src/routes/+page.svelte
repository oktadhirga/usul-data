...
<div class="space-y-6 max-w-7xl mx-auto pb-12">
	...
	<div
		class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-indigo-900/60 via-slate-900 to-slate-900 border border-indigo-500/20 p-6 md:p-8 shadow-xl"
	>
		<div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
			...
			<div class="flex items-center gap-2.5 flex-wrap">
				<Button
					variant="outline"
					size="sm"
					class="border-slate-700 bg-slate-800/80 hover:bg-slate-700 text-slate-200 text-xs h-9 gap-1.5"
					onclick={loadStats}
				...
				<Button
					size="sm"
					class="bg-emerald-600 hover:bg-emerald-500 text-white font-medium text-xs h-9 shadow-md shadow-emerald-600/20 gap-2 transition"
	...
	<Card class="border-slate-800 bg-slate-900/80 backdrop-blur-sm">
		<CardContent class="p-4">
			<div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
				...
				<div class="grid grid-cols-2 sm:grid-cols-4 gap-3 w-full md:w-auto">
					<!-- Filter Tahun -->
					<div>
						<label for="filter-tahun" class="block text-[11px] text-slate-400 mb-1">Tahun</label>
						<select
							id="filter-tahun"
							bind:value={filterTahun}
							onchange={loadStats}
							class="w-full h-8 px-2.5 text-xs rounded-lg bg-slate-800 border border-slate-700 text-slate-200 focus:outline-none focus:border-indigo-500"
						>
					...
					<!-- Filter Bulan -->
					<div>
						<label for="filter-bulan" class="block text-[11px] text-slate-400 mb-1">Bulan</label>
						<select
							id="filter-bulan"
					...
					<!-- Filter Status -->
					<div>
						<label for="filter-status" class="block text-[11px] text-slate-400 mb-1">Status Usulan</label>
						<select
							id="filter-status"
					...
					{#if authState.isAdmin}
						<div>
							<label for="filter-unor" class="block text-[11px] text-slate-400 mb-1">Unit Organisasi</label>
							<select
								id="filter-unor"
								bind:value={filterKodeUnor}
								onchange={loadStats}
								class="w-full h-8 px-2.5 text-xs rounded-lg bg-slate-800 border border-slate-700 text-slate-200 focus:outline-none focus:border-indigo-500 truncate"
							>
					...
					{:else}
						<div class="flex flex-col justify-end">
							<span class="block text-[11px] text-slate-400 mb-1">Lingkup Unit</span>
							<div class="h-8 px-2.5 flex items-center text-xs bg-slate-800/50 border border-slate-800 rounded-lg text-emerald-400 font-mono truncate" title={authState.user?.kodeUnor || ''}>
	...
	{#if errorMessage}
		<div class="p-4 rounded-xl bg-rose-950/40 border border-rose-800 text-rose-300 text-xs flex items-center gap-2">
	...
	<div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
		<!-- Card 1: Total Usulan -->
		<Card class="border-slate-800 bg-slate-900/80 relative overflow-hidden group hover:border-slate-700 transition">
			...
			<CardHeader class="pb-1.5 flex flex-row items-center justify-between">
				<CardTitle class="text-xs font-semibold text-slate-400 uppercase">Total Usulan</CardTitle>
				<FileText class="h-4 w-4 text-indigo-400" />
			</CardHeader>
			<CardContent>
				<div class="text-3xl font-black text-white tracking-tight">
					{#if isLoadingStats}
						<div class="h-8 w-12 bg-slate-800 rounded animate-pulse"></div>
					{:else}
		...
		<Card class="border-amber-900/30 bg-amber-950/20 relative overflow-hidden group hover:border-amber-700/40 transition">
			<CardHeader class="pb-1.5 flex flex-row items-center justify-between">
				<CardTitle class="text-xs font-semibold text-amber-400/90 uppercase">Menunggu</CardTitle>
			...
			<CardContent>
				...
				<div class="flex items-center gap-1 text-[11px] text-amber-500/80 mt-1">
		...
		<!-- Card 3: Disetujui -->
		<Card class="border-emerald-900/30 bg-emerald-950/20 relative overflow-hidden group hover:border-emerald-700/40 transition">
			<CardHeader class="pb-1.5 flex flex-row items-center justify-between">
				<CardTitle class="text-xs font-semibold text-emerald-400/90 uppercase">Disetujui</CardTitle>
				<CheckCircle2 class="h-4 w-4 text-emerald-400" />
			</CardHeader>
			<CardContent>
				<div class="text-3xl font-black text-emerald-300 tracking-tight">
					{#if isLoadingStats}
						<div class="h-8 w-12 bg-slate-800 rounded animate-pulse"></div>
					{:else}
				...
				<div class="flex items-center gap-1 text-[11px] text-emerald-500/80 mt-1">
		...
		<!-- Card 4: Ditolak -->
		<Card class="border-rose-900/30 bg-rose-950/20 relative overflow-hidden group hover:border-rose-700/40 transition">
			<CardHeader class="pb-1.5 flex flex-row items-center justify-between">
				<CardTitle class="text-xs font-semibold text-rose-400/90 uppercase">Ditolak</CardTitle>
				<XCircle class="h-4 w-4 text-rose-400" />
			</CardHeader>
			<CardContent>
				<div class="text-3xl font-black text-rose-300 tracking-tight">
					{#if isLoadingStats}
						<div class="h-8 w-12 bg-slate-800 rounded animate-pulse"></div>
					{:else}
				...
				<div class="flex items-center gap-1 text-[11px] text-rose-500/80 mt-1">
		...
		<!-- Card 5: Draft & Batal -->
		<Card class="border-slate-800 bg-slate-900/70 relative overflow-hidden group hover:border-slate-700 transition col-span-2 sm:col-span-1">
			<CardHeader class="pb-1.5 flex flex-row items-center justify-between">
				<CardTitle class="text-xs font-semibold text-slate-400 uppercase">Draft / Batal</CardTitle>
				<Layers class="h-4 w-4 text-slate-400" />
			</CardHeader>
			<CardContent>
				<div class="text-3xl font-black text-slate-300 tracking-tight">
					{#if isLoadingStats}
						<div class="h-8 w-12 bg-slate-800 rounded animate-pulse"></div>
					{:else}
	...
	<div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
		...
		<Card class="lg:col-span-7 border-slate-800 bg-slate-900/80">
				...
				{:else}
					<div class="overflow-x-auto rounded-lg border border-slate-800">
						<table class="w-full text-xs text-left">
							<thead class="bg-slate-800/80 text-slate-400 uppercase font-semibold text-[10px]">
								<tr>
									<th class="px-3 py-2.5">Unit Organisasi</th>
									<th class="px-2 py-2.5 text-center">Total</th>
									<th class="px-2 py-2.5 text-center text-amber-400">Diajukan</th>
									<th class="px-2 py-2.5 text-center text-emerald-400">Disetujui</th>
									<th class="px-2 py-2.5 text-center text-rose-400">Ditolak</th>
								</tr>
							</thead>
								...
								{#each stats.byUnor as item}
									<tr class="hover:bg-slate-800/40 transition">
										<td class="px-3 py-2 text-slate-200">
											...
											<div class="text-[10px] font-mono text-slate-500">{item.kodeUnor}</div>
										</td>
										<td class="px-2 py-2 text-center font-bold text-white">
											{item.total}
										</td>
										<td class="px-2 py-2 text-center text-amber-400 font-semibold">
											{item.diajukan}
										</td>
										<td class="px-2 py-2 text-center text-emerald-400 font-semibold">
											{item.disetujui}
										</td>
										<td class="px-2 py-2 text-center text-rose-400 font-semibold">
											{item.ditolak}
										</td>
									</tr>
								{/each}
		...
		<Card class="lg:col-span-5 border-slate-800 bg-slate-900/80">
			...
			<CardContent class="space-y-4">
				{#if isLoadingStats}
					<div class="py-12 text-center text-xs text-slate-500 flex flex-col items-center justify-center gap-2">
						<Loader2 class="h-5 w-5 animate-spin text-emerald-400" />
						...
						{#each stats.byKategori as kat}
							...
							<div class="space-y-1">
								<div class="flex items-center justify-between text-xs">
									<span class="font-medium text-slate-300">{kat.kategori}</span>
									<span class="font-mono text-slate-400">{kat.count} usulan ({percentage}%)</span>
								</div>
					...
					<table class="w-full text-xs text-left">
						<thead class="bg-slate-800/80 text-slate-400 uppercase font-semibold text-[10px]">
							<tr>
								...
								<th class="px-2 py-2.5 text-center text-amber-400">Menunggu</th>
								<th class="px-2 py-2.5 text-center text-emerald-400">Disetujui</th>
								<th class="px-2 py-2.5 text-center text-rose-400">Ditolak</th>
						...
						<tbody class="divide-y divide-slate-800/60">
							{#each stats.monthlyTrend as item}
								...
								<tr class={`hover:bg-slate-800/40 transition ${item.total > 0 ? 'bg-slate-900/30' : ''}`}>
									...
									<td class="px-2 py-2 text-center text-amber-400">
										{item.diajukan}
									</td>
									<td class="px-2 py-2 text-center text-emerald-400">
										{item.disetujui}
									</td>
									<td class="px-2 py-2 text-center text-rose-400">
										{item.ditolak}
									</td>
									<td class="px-2 py-2 text-center font-mono text-[11px]">
										{#if finished > 0}
											<span class={successRate >= 70 ? 'text-emerald-400 font-bold' : 'text-slate-400'}>
												{successRate}%
											</span>
										{:else}
		...
		<Card class="border-slate-800 bg-slate-900/60">
			<CardHeader class="flex flex-row items-center justify-between">
				...
				<Button
					variant="outline"
					size="sm"
					class="h-8 gap-1.5 text-xs border-slate-700 bg-slate-800"
					onclick={loadScope}
					...
					<div class="rounded-xl border border-slate-800/80 bg-slate-950/60 p-4 space-y-3">
						<div class="flex items-center justify-between flex-wrap gap-2">
							<div class="flex items-center gap-2">
								{#if scopedInfo.success}
									<CheckCircle2 class="h-4 w-4 text-emerald-400" />
									<span class="text-xs font-semibold text-emerald-400">Scoping Sukses & Aman</span>
								{:else}
						...
						<div class="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs pt-2 border-t border-slate-800/60">
							<div>
								...
								<span class="font-mono text-slate-200">{scopedInfo.userRole || '-'}</span>
							</div>
							<div>
								<span class="text-slate-500 block text-[11px]">Unit Kerja (UNOR):</span>
								<span class="font-medium text-slate-200 block truncate" title={unorName()}>{unorName()}</span>
							</div>
							<div>
								<span class="text-slate-500 block text-[11px]">Scope Aktif:</span>
								<span class="font-mono text-slate-200">{scopedInfo.scopeUnor || '(Global / Semua)'}</span>
							</div>
						</div>
					</div>
				{/if}
