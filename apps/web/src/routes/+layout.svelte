...
{#if isLoginPage}
	{@render children()}
{:else}
	<div class="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col md:flex-row transition-colors">
		<!-- Mobile Header -->
		<div class="md:hidden flex items-center justify-between border-b border-slate-200 dark:border-slate-800 bg-white/90 dark:bg-slate-900/90 px-4 py-3 sticky top-0 z-40 backdrop-blur-md">
			...
			<div class="flex items-center gap-2">
				...
				<button
					type="button"
					class="p-2 rounded-lg text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer"
		...
		<!-- Sidebar -->
		<aside
			class={`fixed inset-y-0 left-0 z-50 w-64 border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/95 p-5 flex flex-col justify-between transition-transform duration-200 md:static md:translate-x-0 ${
		...
		>
			...
			<div class="border-t border-slate-200 dark:border-slate-800/80 pt-4 space-y-3">
				{#if authState.user}
					<div class="flex items-center gap-3 px-2">
						<div class="flex h-9 w-9 items-center justify-center rounded-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 font-semibold text-xs text-indigo-600 dark:text-indigo-400">
							{authState.user.username.slice(0, 2).toUpperCase()}
						</div>
						<div class="flex-1 min-w-0">
							<p class="truncate text-sm font-semibold text-slate-900 dark:text-white leading-tight">
				...
				<Button
					variant="ghost"
					class="w-full justify-start text-rose-600 hover:text-rose-700 hover:bg-rose-50 dark:text-rose-400 dark:hover:text-rose-300 dark:hover:bg-rose-950/40 text-xs h-9"
				...
				<div class="flex items-center gap-4">
					...
					{#if authState.user}
						<div class="text-right">
							<div class="text-xs font-semibold text-slate-900 dark:text-slate-200">{authState.user.username}</div>
							<div class="text-[11px] text-slate-500 dark:text-slate-400">
								{authState.user.role === 'Admin' ? 'Admin Pusat' : (userUnorName() || 'Admin OPD')}
							</div>
						</div>
						<div class="h-8 w-8 rounded-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center text-xs font-bold text-indigo-600 dark:text-indigo-400">
							{authState.user.username.slice(0, 2).toUpperCase()}
						</div>
					{/if}
				</div>
